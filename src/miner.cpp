...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("HYPEMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("HYPEMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("HYPEMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("hype-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        if (fProofOfStake) {
            ...
            if (!pblock->SignBlock(*pwallet)) {
                LogPrintf("HYPEMiner(): Signing new block failed \n");
                continue;
            }
        ...
        LogPrintf("Running HYPEMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("ThreadHYPEMiner() exception");
    } catch (...) {
        LogPrintf("ThreadHYPEMiner() exception");
    }

    LogPrintf("ThreadHYPEMiner exiting\n");
}
