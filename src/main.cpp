...
#if defined(NDEBUG)
#error "Hype cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Hype Signed Message:\n";
...
int64_t GetBlockValue(int nHeight)
{
	...
	if (nHeight == 1) return 352000 * COIN;
		
	int64_t nSubsidy;
	
	if( nHeight > 1 && nHeight <= 3000 ) {
	        nSubsidy = 1 * COIN;
	} else if( nHeight > 3000 && nHeight <= 12000 ) {
	        nSubsidy = 60 * COIN;
